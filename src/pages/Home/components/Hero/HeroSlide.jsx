...
export default function HeroSlide({ manga }) {
    ...
    const handleClick = () => navigate(`/manga/${manga.mal_id}`);
        ...
        <div onClick={handleClick} className="relative min-w-full h-full cursor-pointer">
            ...
            <div className="relative z-10 h-full flex flex-col justify-end md:justify-center lg:justify-start md:flex-row items-start md:items-center gap-2 sm:gap-3 md:gap-8 lg:gap-10 xl:gap-12 px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 2xl:px-20 py-4 sm:py-5 md:py-8">
                <div className="relative shrink-0 hidden md:block group/image">
                    ...
                    <img src={manga?.images?.jpg?.large_image_url ?? defaultPic} alt={manga.title} className="relative md:w-44 md:h-64 lg:w-52 lg:h-76 xl:w-60 xl:h-88 2xl:w-64 2xl:h-96 object-cover rounded-xl sm:rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-white/10 transition-all duration-500 ease-out group-hover/image:-translate-y-2 group-hover/image:shadow-[0_30px_60px_rgba(168,85,247,0.3)]" />
                </div>
                <div className="flex flex-col justify-end md:justify-center items-start gap-2 sm:gap-1.5 md:gap-2 w-full max-w-full md:max-w-xl lg:max-w-2xl">
                    <div className="space-y-0.5 sm:space-y-1 md:space-y-2">
                        <h1 className="text-xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-extrabold text-transparent bg-clip-text bg-linear-to-br from-white via-white to-gray-400 tracking-tight leading-tight line-clamp-2">{manga.title}</h1>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-1">
                        {manga.genres?.slice(0, 4).map((genre) => (
                            <span key={genre.mal_id} className="px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-full bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/20 text-purple-200 text-[10px] sm:text-xs md:text-sm font-medium tracking-wide transition-colors duration-300">{genre.name}</span>
                        ))}
                    </div>
                    <button onClick={(e) => { e.stopPropagation(); navigate(`/manga/${manga.mal_id}`); }} className="mt-2 sm:mt-3 group/btn flex items-center justify-center gap-2 w-full sm:w-fit px-5 sm:px-8 py-2.5 sm:py-3.5 rounded-xl bg-white text-black text-xs sm:text-sm font-bold uppercase tracking-wider shadow-[0_0_20px_rgba(255,255,255,0.2)] cursor-pointer transition-all duration-300 hover:bg-gray-100 hover:shadow-[0_0_30px_rgba(255,255,255,0.4)] hover:-translate-y-1">
                        <span>View Details</span>
                        <HiArrowNarrowRight className="w-4 h-4 transition-transform duration-300 group-hover/btn:translate-x-1" />
                    </button>
                </div>
            </div>
        </div>
