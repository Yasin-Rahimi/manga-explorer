...
export default function ImageDropzone({ previewUrl, onClear, onClick }) {
    return (
        <div
            className="border-2 border-dashed border-gray-500 rounded-xl p-4 sm:p-6 text-center cursor-pointer hover:border-purple-400 transition-colors duration-200"
            onClick={onClick}
        >
            {previewUrl ? (
                <ImagePreview previewUrl={previewUrl} onClear={onClear} />
            ) : (
                <div className="py-4 sm:py-8 text-gray-400">
                    ...
                    <p className="text-xs mt-1 sm:mt-2">PNG, JPG, WEBP up to 5MB</p>
                </div>
            )}
        </div>
    );
