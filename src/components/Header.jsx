    ...
    return (
        <header className="sticky top-0 z-50 w-full border-b border-white/5 bg-black/30 backdrop-blur-xl shadow-lg shadow-black/20">
            <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-5 px-4 sm:px-6 md:px-8 py-3 sm:py-4">
                <Link to="/" className="flex items-center gap-2 sm:gap-3 group transition-transform hover:scale-[1.02] w-full sm:w-auto">
                ...
                {isHome ? (
                    <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                        <div className="relative group w-full">
                            ...
                            <input
                                ...
                                className={`w-full min-w-0 sm:w-70 md:w-[320px] lg:w-90 pl-10 pr-4 py-2.5 sm:py-3 text-sm sm:text-base rounded-xl bg-white/5 border transition-all duration-300 shadow-inner focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:bg-white/10 ${isEmpty && submitClicked ? "border-red-500/70 focus:border-red-400" : "border-white/10 focus:border-purple-500/50"} text-white placeholder-gray-400`}
                            />
                        </div>
                        <button className="w-full sm:w-auto whitespace-nowrap bg-linear-to-r from-purple-600 to-indigo-600 px-5 sm:px-6 py-2.5 sm:py-3 rounded-xl text-sm sm:text-base text-white font-medium cursor-pointer hover:from-purple-500 hover:to-indigo-500 focus:ring-2 focus:ring-purple-500/50 focus:outline-none shadow-lg shadow-purple-500/25 hover:shadow-purple-500/40 transition-all duration-300 transform active:scale-95">
                            Search
                        </button>
                    </form>
                ) : (
